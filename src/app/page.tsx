...
export default function Home() {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <header className="flex items-center justify-between border-b-3 border-black bg-white dark:bg-slate-900 px-6 h-[76px] sticky top-0 z-50 shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-4">
            <div className="size-10 bg-primary comic-border-sm flex items-center justify-center text-white hidden sm:flex">
        ...
        <div className="flex gap-4">
          <ThemeToggle />
          <button className="comic-button comic-border-sm bg-white dark:bg-slate-800 p-2 flex items-center justify-center hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors hidden sm:flex">
      ...
      <div className="flex flex-1 relative">
        {/* Sidebar Overlay */}
      <div 
        className={`lg:hidden fixed inset-0 top-[76px] bg-slate-900/60 z-40 transition-opacity duration-300 ${isSidebarOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
        onClick={() => setIsSidebarOpen(false)}
      />

      {/* Sidebar */}
      <aside 
        className={`bg-white dark:bg-slate-900 flex flex-col fixed lg:sticky top-[76px] h-[calc(100vh-76px)] z-40 shrink-0 left-0 transition-all duration-300 ease-in-out overflow-hidden border-black ${
          isSidebarOpen ? "w-[280px] translate-x-0 border-r-3" : "w-[280px] lg:w-0 -translate-x-full lg:translate-x-0 border-r-0"
        }`}
      >
        <div className="w-[280px] flex flex-col h-full shrink-0">
          <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
            <button className="w-full flex items-center gap-3 px-4 py-3 font-bold uppercase transition-colors bg-primary text-white">
              <span className="material-symbols-outlined">dashboard</span>
              Dashboard
            </button>
            <button className="w-full flex items-center gap-3 px-4 py-3 font-bold uppercase transition-colors text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-slate-100">
          ...
          <div className="p-4 border-t-3 border-black space-y-2 bg-slate-50 dark:bg-slate-900/50 shrink-0">
            <button className="w-full flex items-center gap-3 px-4 py-3 font-bold uppercase transition-colors text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-slate-100">
              <span className="material-symbols-outlined">settings</span>
              Settings
            </button>
            <button className="w-full flex items-center gap-3 px-4 py-3 font-bold uppercase transition-colors text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20">
                ...
                <div className="flex-1 min-w-[150px]">
                  ...
                  <div className="relative h-[44px]">
                    <button 
                      ...
                      className={`w-full h-full comic-button comic-border-sm bg-slate-100 dark:bg-slate-800 border-black hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors outline-none flex items-center justify-between px-4 font-bold uppercase text-slate-900 dark:text-slate-100`}
                    >
                      <span className="truncate">{statusVal}</span>
                      <span className={`material-symbols-outlined pointer-events-none text-slate-500 dark:text-slate-400 transition-transform duration-200 ${isStatusOpen ? 'rotate-180' : ''}`}>
                        expand_more
                      </span>
                    </button>
                    <div 
                      className={`absolute top-full left-0 right-0 mt-2 comic-border-sm bg-white dark:bg-slate-800 border-black z-50 origin-top transition-all duration-200 ease-out ${isStatusOpen ? 'opacity-100 scale-100 visible' : 'opacity-0 scale-95 invisible'}`}
                    >
                      <ul className="py-2 text-sm font-bold uppercase text-slate-900 dark:text-slate-100">
                ...
                <div className="flex-1 min-w-[150px]">
                  ...
                  <div className="relative h-[44px]">
                    ...
                    >
                      ...
                      <span className={`material-symbols-outlined pointer-events-none text-slate-500 dark:text-slate-400 transition-transform duration-200 ${isDateRangeOpen ? 'rotate-180' : ''}`}>
                        expand_more
                      </span>
                    </button>
                    <div 
                      className={`absolute top-full left-0 right-0 mt-2 comic-border-sm bg-white dark:bg-slate-800 border-black z-50 origin-top transition-all duration-200 ease-out ${isDateRangeOpen ? 'opacity-100 scale-100 visible' : 'opacity-0 scale-95 invisible'}`}
                    >
                      <ul className="py-2 text-sm font-bold uppercase text-slate-900 dark:text-slate-100">
