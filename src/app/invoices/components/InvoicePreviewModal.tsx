...
export function InvoicePreviewModal({
  ...
  return (
    <div role="dialog" aria-modal="true" className="modal-surface relative max-w-3xl p-5 sm:p-7 max-h-[90vh] overflow-y-auto animate-in fade-in-50 zoom-in-95 duration-200">
      ...
      <div className="surface-card p-5 border border-dashed border-[var(--accent)]/[0.25] bg-[var(--accent)]/[0.01] rounded-2xl relative overflow-hidden backdrop-blur-md mt-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {/* Webhook log stream */}
          <div className="bg-[var(--foreground)]/[0.03] border border-[var(--card-border)] rounded-xl p-3 flex flex-col h-[236px]">
            <span className="text-[9px] font-bold text-[var(--muted)] tracking-wider uppercase mb-2">Simulated Live Log Stream</span>
            <div className="flex-1 overflow-y-auto font-mono text-[9.5px] leading-relaxed text-[var(--muted)] space-y-1.5 custom-scrollbar select-text selection:bg-[var(--accent)]/20">
              {webhookLogs.map((log, index) => (
                <div key={index} className="border-b border-[var(--card-border)]/10 pb-1 last:border-0">{log}</div>
              ))}
            </div>
          </div>
        </div>
